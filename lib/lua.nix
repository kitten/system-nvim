# See: https://github.com/nix-community/nixvim/blob/3a66c8a33001d8bd79388c6b15eb1039f43f4192/lib/to-lua.nix
{ lib, ... }:
...
rec {
  ...
  mkInline = v: generators.mkLuaInline (v.__raw or v);
    ...
    let
      ...
      toTableKey =
        ...
        if allowRawAttrKeys && hasPrefix "__rawKey__" s then
          ...
          trace ''lib.lua.toLua: __emptyString is deprecated, just use an attribute named "".'' (
            toTableKey ""
          )
        else
          "[${go "" s}]";
        ...
        # It can output an exponent form supported by lua.
          ...
          # apply raw values if allowed
            ...
            + concatStringsSep ("," + introSpace) (
              mapAttrsToList (
                name: value:
                (if allowUnkeyedAttrs && hasPrefix "__unkeyed" name then "" else toTableKey name + " = ")
          ...
          abort "lib.lua.toLua: should never happen (v = ${v})";
    in
